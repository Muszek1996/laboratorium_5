#include<stdlib.h>
#include<stdio.h>
#include<pthread.h>
#include <unistd.h>
#include <math.h>

#include "include/pomiar_czasu.h"

#define N 100000
#define LICZBA_W 2
#define M_PI 3.14159265358979323846

pthread_mutex_t muteks; 
pthread_t watki[LICZBA_W];
double global_array_of_local_sums[LICZBA_W];

double f(double x){
  return sin(x);
}

double integralSeq(double a, double b) {
    double x1 = a;
    double c = 0.0;
    double dx = (b - a) / N;

    double fx1 = f(x1);

    for (int i = 0; i < N; i++) {


        double x2 = x1 + dx;
        double fx2 = f(x2);
        c += (fx1 + fx2);
        fx1= fx2;
        x1 = x2;
    }
    c*= dx*0.5;

    printf("Wynik całki dla N:%d, dx:%f, wynik: %f\n",N,dx,c);
}



void *calkaMutex(void *arg_wsk);
void *suma_w_no_mutex( void *arg_wsk);

double *tab;
double suma=0;

int main( int argc, char *argv[] ){

  int i; 

  double t1,t2,t3;

  int indeksy[LICZBA_W]; 
  for(i=0;i<LICZBA_W;i++) indeksy[i]=i;

  tab = (double *) malloc(N*sizeof(double));
  for(i=0; i<N; i++ ) tab[i] = ((double) i+1) / N;

  printf("Liczba watkow : %d\n", LICZBA_W);


  pthread_mutex_init( &muteks, NULL);

  printf("Poczatek tworzenia watkow\n");
  t1 = czas_zegara();

  for(i=0; i<LICZBA_W; i++ )
      pthread_create(&watki[i], NULL, calkaMutex, (void *) &indeksy[i]);

  for(i=0; i<LICZBA_W; i++ ) pthread_join( watki[i], NULL );

  t1 = czas_zegara() - t1;
  printf("suma = %lf\n", suma);
  printf("Czas obliczen = %lf\n", t1);

  // version with array of local sums (to eliminate the calkaMutex)
//  suma =0;
//  printf("Poczatek tworzenia watkow\n");
//  t1 = czas_zegara();
//
//  for(i=0; i<LICZBA_W; i++ ) {
//    global_array_of_local_sums[i]=0.0;
//    pthread_create( &watki[i], NULL, suma_w_no_mutex, (void *) &indeksy[i] );
//  // synchronization error !!!
//    //pthread_create( &watki[i], NULL, calkaMutex, (void *) &i );
//  }
//
//  for(i=0; i<LICZBA_W; i++ ) {
//    pthread_join( watki[i], NULL );
//    suma += global_array_of_local_sums[i];
//  }
//
//  t1 = czas_zegara() - t1;
//  printf("suma = %lf\n", suma);
//  printf("Czas obliczen (no calkaMutex) = %lf\n", t1);

/*   suma =0; */

/*   printf("Poczatek tworzenia watkow OpenMP\n"); */
/*   t1 = czas_zegara(); */

/* #pragma omp parallel for num_threads(LICZBA_W) default(none) shared (tab) reduction(+:suma)  */
/*   for(i=0;i<N;i++){ */
/*     suma += tab[i]; */
/*   } */



/*   t1 = czas_zegara() - t1; */
/*   printf("suma = %lf\n", suma); */
/*   printf("Czas obliczen OpenMP = %lf\n", t1); */

}


void *calkaMutex(void *arg_wsk){


    int i, j, moj_id;

    double moja_suma=0;

    moj_id = *( (int *) arg_wsk );

    j=N/LICZBA_W+1;
    double dx = (M_PI - 0) / N;

    double my_start = moj_id*dx * j;
    double my_end = (moj_id+1)*dx * j;
    if(my_end > M_PI) my_end = M_PI;

    for(double k=my_start;k<my_end;k+=dx){
        moja_suma += 0.5*dx*(f(k)+f(k+dx));
    }

    pthread_mutex_lock( &muteks );
    suma += moja_suma;
    pthread_mutex_unlock( &muteks );

    pthread_exit( (void *)0);

}

void *suma_w_no_mutex( void *arg_wsk){

  int i, j, moj_id;

  moj_id = *( (int *) arg_wsk ); 

  j=N/LICZBA_W;
  for( i=j*moj_id+1; i<=j*(moj_id+1); i++){ 
    global_array_of_local_sums[moj_id] += tab[i]; 
  }

  pthread_exit( (void *)0);

}
